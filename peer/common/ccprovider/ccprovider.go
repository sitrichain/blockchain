...
// enables it.
// TODO: (post v1) enable cache by default as soon as https://jira.rongzer.org/browse/FAB-3785 is completed
...
func GetChaincodeData(ccname string, ccversion string) (*ChaincodeData, error) {
	if ccInfoCacheEnabled {
		log.Logger.Debugf("Getting chaincode data for <%s, %s> from cache", ccname, ccversion)
		return ccInfoCache.GetChaincodeData(ccname, ccversion)
	}
	if ccpack, err := ccInfoFSProvider.GetChaincode(ccname, ccversion); err != nil {
		return nil, err
	} else {
		log.Logger.Infof("Putting chaincode data for <%s, %s> into cache", ccname, ccversion)
...
func CheckInsantiationPolicy(name, version string, cdLedger *ChaincodeData) error {
	...
	// https://jira.rongzer.org/browse/FAB-3156
	if ccdata != nil && ccdata.InstantiationPolicy != nil {
	...
	for _, file := range files {
		...
		if len(fileNameArray) == 2 {
			...
			if err != nil {
				...
				log.Logger.Errorf("Unreadable chaincode file found on filesystem: %s", file.Name())
				continue
			}
			...
			if name != ccname || version != ccversion {
				...
				log.Logger.Errorf("Chaincode file's name/version has been modified on the filesystem: %s", file.Name())
				continue
			}
...
func NewCCContext(cid, name, version, txid string, syscc bool, signedProp *pb.SignedProposal, prop *pb.Proposal) *CCContext {
	...
	log.Logger.Debugf("NewCCCC (chain=%s,chaincode=%s,version=%s,txid=%s,syscc=%t,proposal=%p,canname=%s", cid, name, version, txid, syscc, prop, cccid.canonicalName)
...
type ChaincodeData struct {
	...
	// 保存buf for the chaincode
	Buf []byte `protobuf:"bytes,9,opt,name=buf,proto3"`

	TxID string `protobuf:"bytes,10,opt,name=txId"`

	Timestamp *types.Timestamp `protobuf:"bytes,11,opt,name=timestamp" json:"timestamp,omitempty"`

	Desc string `protobuf:"bytes,12,opt,name=desc"`

	//编译结果,仅JAVA合约使用
	CBuf []byte `protobuf:"bytes,13,opt,name=cbuf,proto3"`

	//创建的会员,加密使用
	CustomerNo string `protobuf:"bytes,14,opt,name=customerNo,proto3"`

	//合约别名
	Alias string `protobuf:"bytes,15,opt,name=alias"`
}
...
type ChaincodeProvider interface {
	...
	LockTxsim()
	UnLockTxsim()
	GetTxsim() ledger.TxSimulator
	TxsimDone(txsim ledger.TxSimulator)
}
