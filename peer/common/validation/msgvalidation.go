...
func validateConfigTransaction(data []byte, hdr *common.Header) error {
	log.Logger.Debugf("validateConfigTransaction starts for data %p, header %s", data, hdr)
...
func ValidateTransaction(e *common.Envelope) (*common.Payload, pb.TxValidationCode) {
	log.Logger.Debugf("ValidateTransactionEnvelope starts for envelope %p", e)

	// check for nil argument
	if e == nil {
		log.Logger.Errorf("Error: nil envelope")
	...
	if err != nil {
		log.Logger.Errorf("GetPayload returns err %s", err)
		return nil, pb.TxValidationCode_BAD_PAYLOAD
	}

	log.Logger.Debugf("Header is %s", payload.Header)

	// validate the header
	chdr, shdr, err := validateCommonHeader(payload.Header)
	if err != nil {
		log.Logger.Errorf("validateCommonHeader returns err %s", err)
	...
	if err != nil {
		log.Logger.Errorf("CheckSignatureFromCreator returns err %s", err)
	...
	case common.HeaderType_ENDORSER_TRANSACTION:
		...
		/*
			...
			if err != nil {
				log.Logger.Errorf("GetTransaction returns err %s", err)
			}

			if tx != nil && len(tx.Actions) > 0 {

				txActions := tx.Actions[0]
				log.Logger.Infof("get txActions from txAction %v", txActions)

				hdr1, _ := utils.GetHeader(txActions.Header)
				if hdr1 != nil {
					log.Logger.Infof("get hdr1 from txAction %v", hdr1)

					shdr1, err := utils.GetSignatureHeader(hdr1.SignatureHeader)
					if shdr1 != nil {
						log.Logger.Infof("get shdr from txAction[0] %v,err:%s", shdr1, err)
						shdr = shdr1
					}
				}
			}
			...
			if err != nil {
				log.Logger.Errorf("CheckProposalTxID returns err %s", err)
				return nil, pb.TxValidationCode_BAD_PROPOSAL_TXID
			}
		*/
		err = validateEndorserTransaction(payload.Data, payload.Header)
		log.Logger.Debugf("ValidateTransactionEnvelope returns err %s", err)
	...
	case common.HeaderType_CONFIG:
		...
		if err != nil {
			log.Logger.Errorf("validateConfigTransaction returns err %s", err)
