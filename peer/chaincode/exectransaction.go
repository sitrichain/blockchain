...
func Execute(ctxt context.Context, cccid *ccprovider.CCContext, spec interface{}) (*pb.Response, *pb.ChaincodeEvent, error) {
	...
	if resp.Type == pb.ChaincodeMessage_COMPLETED {
		res := &pb.Response{}
		if err := res.Unmarshal(resp.Payload); err != nil {
			return nil, nil, fmt.Errorf("Failed to unmarshal response for (%s): %s", cccid.TxID, err)
		}
...
func ExecuteWithErrorFilter(ctxt context.Context, cccid *ccprovider.CCContext, spec interface{}) ([]byte, *pb.ChaincodeEvent, error) {
	...
	if err != nil {
		log.Logger.Errorf("ExecuteWithErrorFilter %s error: %s", cccid.Name, err)
		return nil, nil, err
	}

	if res == nil {
		log.Logger.Errorf("ExecuteWithErrorFilter %s get nil response without error", cccid.Name)
		return nil, nil, err
	}
