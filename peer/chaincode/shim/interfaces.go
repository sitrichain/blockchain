...
// Chaincode interface must be implemented by all chaincodes. The blockchain runs
...
type ChaincodeStubInterface interface {
	...
	GetTxTimestamp() (*types.Timestamp, error)
	...
	GetHeightById(rlistName string, id string) int
	GetIdsByHeight(rlistName string, height int) ([]string, int)

	// RCounter的相关方法
	GetValue(rCounterName string) int64
	SetValue(rCounterName string, lValue int64)
	AddValue(rCounterName string, lValue int64)
	MulValue(rCounterName string, lValue int64)
	GetCurrentValue(rCounterName string) string
