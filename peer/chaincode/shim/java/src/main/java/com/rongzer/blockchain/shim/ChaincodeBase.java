...
import static com.rongzer.blockchain.shim.Chaincode.Response.Status.INTERNAL_SERVER_ERROR;
import static com.rongzer.blockchain.shim.Chaincode.Response.Status.SUCCESS;
...
import com.rongzer.blockchain.protos.peer.ChaincodeSupportGrpc;
import com.rongzer.blockchain.protos.peer.ChaincodeSupportGrpc.ChaincodeSupportStub;
import com.rongzer.blockchain.shim.impl.Handler;
import com.rongzer.blockchain.shim.impl.NextStateInfo;
...
public abstract class ChaincodeBase implements Chaincode {
	...
	protected static Log logger = LogFactory.getLog(ChaincodeBase.class);
	...
	private String rootCertFile = "/etc/rongzer/blockchain/peer.crt";
	...
	public void processCommandLineOptions(String[] args) {
	...
	public ManagedChannel newPeerClientConnection() {
        ...
        try {
        	...
        	builder.maxInboundMessageSize( 100 * 1024 * 1024);
	...
	public void chatWithPeer(ManagedChannel connection) {
		...
		try {
			requestObserver = stub.register(new StreamObserver<ChaincodeMessage>() {
				...
				public void onNext(ChaincodeMessage message) {
					...
					try {
						logger.debug(String.format("[%-8s]Received message %s from com.rongzer.blockchain.shim", message.getTxid(), message.getType()));
				...
				public void onError(Throwable e) {
					e.printStackTrace();
					logger.error("connect on peer server onError: " + e.getMessage());
					System.exit(-1);
				}
		...
		// Create the com.rongzer.blockchain.shim handler responsible for all
		// control logic
			...
			try {
				...
				if (nsInfo.sendToCC || message.getType() == Type.KEEPALIVE) {
					if (message.getType() == Type.KEEPALIVE) {
						...
						logger.debug(String.format("[%-8s]Send state message %s", message.getTxid(), message.getType()));
					}
