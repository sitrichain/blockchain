...
import (
	...
	"github.com/rongzer/blockchain/peer/chaincode/platforms/car"
	"github.com/rongzer/blockchain/peer/chaincode/platforms/golang"
	"github.com/rongzer/blockchain/peer/chaincode/platforms/java"
...
func getPeerTLSCert() ([]byte, error) {
	...
	path = conf.V.TLS.RootCAs[0]
	if len(conf.V.TLS.RootCAs) == 0 {
		// check for tls cert
		path = conf.V.TLS.Certificate
	}
...
func generateDockerfile(platform Platform, cds *pb.ChaincodeDeploymentSpec, tls bool) ([]byte, error) {
	...
	if tls {
		const guestTLSPath = "/etc/rongzer/blockchain/peer.crt"
