	...
	for i, file := range files {
		go func(i int, file SourceDescriptor) {
			...
			if excluded == false {
				...
				log.Logger.Debugf("vendoring %s -> %s", origName, file.Name)
			}
...
func (goPlatform *Platform) GetDeploymentPayload(spec *pb.ChaincodeSpec) ([]byte, error) {
	...
	var provided = map[string]bool{
		"github.com/rongzer/blockchain/core/chaincode/shim": true,
		"github.com/rongzer/blockchain/protos/peer":         true,
	}

	imports = filter(imports, func(pkg string) bool {
		// Drop if provided by CCENV
		if _, ok := provided[pkg]; ok == true {
			log.Logger.Debugf("Discarding provided package %s", pkg)
			return false
		}
		...
		for goroot := range goroots {
			...
			if err == nil && exists {
				log.Logger.Debugf("Discarding GOROOT package %s", pkg)
				return false
			}
		}

		// Else, we keep it
		log.Logger.Debugf("Accepting import: %s", pkg)
		return true
	})
	...
	for dep := range deps {
		...
		log.Logger.Debugf("processing dep: %s", dep)
		...
		for gopath := range gopaths {
			...
			log.Logger.Debugf("checking: %s exists: %v", fqp, exists)
...
func (goPlatform *Platform) GenerateDockerfile(_ *pb.ChaincodeDeploymentSpec) (string, error) {

	var buf []string

	buf = append(buf, "FROM "+conf.V.Peer.Chaincode.GolangRuntime)
