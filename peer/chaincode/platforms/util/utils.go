...
func ComputeHash(contents []byte, hash []byte) []byte {
	...
	//compute new hash
	hash = util.WriteAndSumSha256(newSlice)
...
func HashFilesInDir(rootDir string, dir string, hash []byte, tw *tar.Writer) ([]byte, error) {
	...
	log.Logger.Debugf("hashFiles %s", currentDir)
	...
	for _, fi := range fis {
		...
		if err != nil {
			log.Logger.Errorf("Error reading %s\n", err)
			return hash, err
		}
...
func DockerBuild(opts DockerBuildOptions) error {
	...
	if err != nil {
		log.Logger.Debugf("Image %s does not exist locally, attempt pull", opts.Image)
