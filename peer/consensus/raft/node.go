...
func (n *node) start(fresh, join bool) {
	raftPeers := RaftPeers(n.metadata.ConsenterEndpoints)
...
func (n *node) send(msgs []raftpb.Message) {
	...
	for _, msg := range msgs {
		...
		if err := n.rpc.SendConsensus(msg.To, &orderer.ConsensusRequest{
			Channel: n.chainID, Payload: msgBytes, SourceNodeInfo: &orderer.NodeInfo{Id: msg.From, EndPoint: conf.V.Sealer.Raft.EndPoint}}); err != nil {
...
func (n *node) logSendFailure(dest uint64, err error) {
	if _, ok := n.unreachable[dest]; ok {
		n.logger.Errorf("Failed to send Consensus Request to %d, because: %s", dest, err)
		return
	}
