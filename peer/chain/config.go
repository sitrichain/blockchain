...
func CacheConfiguration() (err error) {
	...
	getLocalAddress := func() (peerAddress string, err error) {
		if viper.GetBool("peer.addressAutoDetect") {
			...
			log.Logger.Infof("Auto detected peer address: %s", peerAddress)
		} else {
	...
	_, localAddressError = getLocalAddress()
...
func cacheConfiguration() {
	if err := CacheConfiguration(); err != nil {
		log.Logger.Errorf("Execution continues after CacheConfiguration() failure : %s", err)
	}
}
