	...
	for _, d := range block.Data.Data {
		...
		if ebytes != nil {
			if env, err := utils.GetEnvelopeFromBlock(ebytes); err != nil {
				log.Logger.Errorf("error getting tx from block(%s)\n", err)
			} else if env != nil {
				...
				if common.HeaderType(chdr.Type) == common.HeaderType_ENDORSER_TRANSACTION {
					log.Logger.Debugf("Channel [%s]: Block event for block number [%d] contains transaction id: %s", channelId, block.Header.Number, chdr.TxId)
...
func CreateRejectionEvent(chainId string, txId string, errorMsg string) *pb.Event {
	return &pb.Event{Event: &pb.Event_Rejection{Rejection: &pb.Rejection{ChainId: chainId, TxId: txId, ErrorMsg: errorMsg}}}
}
