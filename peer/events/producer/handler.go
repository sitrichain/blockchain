...
func getInterestKey(interest pb.Interest) string {
	...
	case pb.EventType_SUCCESS:
		key = "/" + strconv.Itoa(int(pb.EventType_SUCCESS))
	...
	default:
		log.Logger.Errorf("unknown interest type %s", interest.EventType)
	}
...
func (d *handler) register(iMsg []*pb.Interest) error {
	...
	for _, v := range iMsg {
		if err := registerHandler(v, d); err != nil {
			log.Logger.Errorf("could not register %s: %s", v, err)
			continue
		}
...
func (d *handler) deregister(iMsg []*pb.Interest) error {
	for _, v := range iMsg {
		if err := deRegisterHandler(v, d); err != nil {
			log.Logger.Errorf("could not deregister %s", v)
			continue
		}
...
func validateEventMessage(signedEvt *pb.SignedEvent) (*pb.Event, error) {
	log.Logger.Debugf("ValidateEventMessage starts for signed event %p", signedEvt)
