...
func (hl *genericHandlerList) add(_ *pb.Interest, h *handler) (bool, error) {
	if h == nil {
...
func (hl *genericHandlerList) del(_ *pb.Interest, h *handler) (bool, error) {
	hl.Lock()
	if _, ok := hl.handlers[h]; !ok {
		hl.Unlock()
...
func (hl *genericHandlerList) foreach(_ *pb.Event, action func(h *handler)) {
	hl.Lock()
...
func AddEventType(eventType pb.EventType) error {
	...
	log.Logger.Debugf("Registering %s", pb.EventType_name[int32(eventType)])
...
func registerHandler(ie *pb.Interest, h *handler) error {
	log.Logger.Debugf("registering event type: %s", ie.EventType)
	gEventProcessor.Lock()
...
func deRegisterHandler(ie *pb.Interest, h *handler) error {
	log.Logger.Debugf("deregistering event type: %s", ie.EventType)
...
func Send(e *pb.Event) error {
	...
	if e.Event == nil {
		log.Logger.Error("event not set")
