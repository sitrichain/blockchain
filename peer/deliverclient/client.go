...
func (bc *broadcastClient) try(action func() (interface{}, error)) (interface{}, error) {
	...
	for retry && !bc.shouldStop() {
		...
		if err != nil {
			log.Logger.Errorf("connection has err :%s", err)
...
func (bc *broadcastClient) connect() error {
	log.Logger.Infof("connect to orderer for Deliver")
	conn, endpoint, err := bc.prod.NewConnection()
	if err != nil {
		log.Logger.Error("Failed obtaining connection:", err)
		return err
	}
	...
	if err != nil {
		log.Logger.Error("Connection to ", endpoint, "established but was unable to create gRPC stream:", err)
		conn.Close()
		return err
	}
...
func (bc *broadcastClient) afterConnect(conn *grpc.ClientConn, abc orderer.AtomicBroadcast_DeliverClient, cf context.CancelFunc) error {
	...
	log.Logger.Error("Failed setting up broadcast:", err)
	return err
}
