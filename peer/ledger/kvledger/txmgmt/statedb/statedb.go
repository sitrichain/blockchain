...
func (batch *UpdateBatch) Clear() {
	//遍历清空update
	for uk, uv := range batch.updates {
		for k1 := range uv.m {
			delete(uv.m, k1)
		}
		uv.m = nil
		delete(batch.updates, uk)
	}
	batch.updates = nil

	for ck, cv := range batch.cals {
		for k1 := range cv.m {
			delete(cv.m, k1)
		}
		cv.m = nil
		delete(batch.cals, ck)
	}
	batch.cals = nil

	for rk, rv := range batch.rLists {
		for k1, v1 := range rv.m {
			v1.Clear()
			delete(rv.m, k1)
		}
		delete(batch.rLists, rk)
	}
