...
func (h *queryHelper) getState(ns string, key string) ([]byte, error) {
	...
	if strings.HasPrefix(key, "__RLIST_") {
		...
		//log.Logger.Infof("key : %s lisParamStr : %s lisMethod : %s lisParam : %s ", key, lisParamStr, lisMethod, lisParam)
		...
		} else if strings.EqualFold(lisMethod, "HGT") {
			height := rList.GetIdHeight(lisParam[1])
			val = []byte(strconv.Itoa(height))
		} else if strings.EqualFold(lisMethod, "IDH") {
			b, err := strconv.Atoi(lisParam[1])
			if err != nil {
				b = 0
			}
			ids, height := rList.GetIdsByHeight(b)
			rListHeight := &common.RListHeight{ids, uint64(height)}
			val, _ = rListHeight.Marshal()
		}
		return val, nil
	}
...
func (itr *queryResultsItr) Next() (commonledger.QueryResult, error) {
	...
	log.Logger.Debugf("queryResultsItr.Next() returned a record:%s", string(versionedQueryRecord.Value))
