...
func newLockBasedTxSimulator(txmgr *LockBasedTxMgr) *lockBasedTxSimulator {
	...
	log.Logger.Debugf("constructing new tx simulator [%s]", id)
...
func (s *lockBasedTxSimulator) GetTxSimulationResults() ([]byte, error) {
	log.Logger.Debugf("Simulation completed, getting simulation results")
	s.Done()
	if s.helper.err != nil {
...
func (s *lockBasedTxSimulator) ExecuteUpdate(_ string) error {
