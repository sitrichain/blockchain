...
func (le *leaderElectionSvcImpl) handleMessages() {
	log.Logger.Debug(le.id, ": Entering")
	defer log.Logger.Debug(le.id, ": Exiting")
	...
	for {
		...
		case msg := <-msgChan:
			if !le.isAlive(msg.SenderID()) {
				log.Logger.Debug(le.id, ": Got message from", msg.SenderID(), "but it is not in the view")
				break
			}
...
func (le *leaderElectionSvcImpl) handleMessage(msg Msg) {
	...
	log.Logger.Debug(le.id, ":", msg.SenderID(), "sent us", msgType)
	le.Lock()
	defer le.Unlock()
	...
	if msg.IsProposal() {
		...
		log.Logger.Error("Got a message that's not a proposal and not a declaration")
	}
}
...
func (le *leaderElectionSvcImpl) leaderElection() {
	...
	if le.isLeaderExists() {
		log.Logger.Info(le.id, ": Some peer is already a leader")
		return
	}

	if le.isYielding() {
		log.Logger.Debug(le.id, ": Aborting leader election because yielding")
		return
	}
...
func (le *leaderElectionSvcImpl) propose() {
	log.Logger.Debug(le.id, ": Entering")
	log.Logger.Debug(le.id, ": Exiting")
...
func (le *leaderElectionSvcImpl) waitForMembershipStabilization(timeLimit time.Duration) {
	log.Logger.Debug(le.id, ": Entering")
	defer log.Logger.Debug(le.id, ": Exiting, peers found", len(le.adapter.Peers()))
...
func (le *leaderElectionSvcImpl) IsLeader() bool {
	...
	log.Logger.Debug(le.id, ": Returning", isLeader)
	return isLeader
}

func (le *leaderElectionSvcImpl) beLeader() {
	log.Logger.Info(le.id, ": Becoming a leader")
	atomic.StoreInt32(&le.isLeader, int32(1))
	le.callback(true)
}

func (le *leaderElectionSvcImpl) stopBeingLeader() {
	log.Logger.Info(le.id, "Stopped being a leader")
...
func (le *leaderElectionSvcImpl) stopYielding() {
	log.Logger.Debug("Stopped yielding")
	le.Lock()
	defer le.Unlock()
...
func getStartupGracePeriod() time.Duration {
	return util2.GetDurationOrDefault("peer.gossip.election.startupGracePeriod", time.Second*15)
}

func getMembershipSampleInterval() time.Duration {
	return util2.GetDurationOrDefault("peer.gossip.election.membershipSampleInterval", time.Second)
}

func getLeaderAliveThreshold() time.Duration {
	return util2.GetDurationOrDefault("peer.gossip.election.leaderAliveThreshold", time.Second*10)
}

func getLeadershipDeclarationInterval() time.Duration {
	return getLeaderAliveThreshold() / 2
}

func getLeaderElectionDuration() time.Duration {
	return util2.GetDurationOrDefault("peer.gossip.election.leaderElectionDuration", time.Second*5)
}
