...
// 缓存会员编号和证书
var MemberCertData = struct {
	sync.RWMutex
	MapBuf map[string]string
...
func (s *GossipStateProviderImpl) listen() {
	...
	for {
		...
		case msg := <-s.gossipChan:
			log.Logger.Debug("Received new message via gossip channel")
			go s.queueNewMessage(msg)
		case msg := <-s.commChan:
			log.Logger.Debugf("Received direct message %s", msg)
			go s.directMessage(msg)
		case <-s.stopCh:
			s.stopCh <- struct{}{}
			log.Logger.Warn("Stop listening for new messages")
			return
		}
	}
}
...
func (s *GossipStateProviderImpl) directMessage(msg proto.ReceivedMessage) {
	...
	if incoming.GetStateRequest() != nil {
		...
		// 增加了对LightweightRequest的处理
	} else if incoming.GetLightweightRequest() != nil {
		if len(s.stateRequestCh) < defChannelBufferSize {
			s.stateRequestCh <- msg
		}
	}
...
func (s *GossipStateProviderImpl) handleStateRequest(msg proto.ReceivedMessage) {
	...
	// 若请求不是StateRequest类型，则跳转到handleLightWeightStateRequest
	if request == nil {
		s.handleLightWeightStateRequest(msg)
		return
	}
	log.Logger.Warnf("handleStateRequest connectionInfo : %s msg : %s", msg.GetConnectionInfo(), msg)

	if request.StartSeqNum > request.EndSeqNum {
		log.Logger.Errorf("Invalid sequence interval [%d...%d], ignoring request...", request.StartSeqNum, request.EndSeqNum)
		return
	}

	currentHeight, err := s.committer.LedgerHeight()
	if err != nil {
		log.Logger.Errorf("Cannot access to current ledger height, due to %s", err)
		return
	}
	if currentHeight < request.EndSeqNum {
		log.Logger.Warnf("Received state request to transfer blocks with sequence numbers higher  [%d...%d] "+
	...
	//log.Logger.Warnf("Reading block to peer %d to %d", request.StartSeqNum, endSeqNum)

	response := &proto.RemoteStateResponse{Payloads: make([]*proto.Payload, 0)}
	for seqNum := request.StartSeqNum; seqNum <= endSeqNum; seqNum++ {
		log.Logger.Debug("Reading block ", seqNum, " from the committer service")
		blocks := s.committer.GetBlocks([]uint64{seqNum})

		if len(blocks) == 0 {
			log.Logger.Errorf("Wasn't able to read block with sequence number %d from ledger, skipping....", seqNum)
			continue
		}

		blockBytes, err := pb.Marshal(blocks[0])
		if err != nil {
			log.Logger.Errorf("Could not marshal block: %s", err)
		}
	...
	log.Logger.Warnf("handleStateRequest response msg connectionInfo : %s", msg.GetConnectionInfo())
}

func filterBlock(block *common.Block, certsList []string) *common.Block {
	// 创世区块不进行过滤
	if block.Header.Number == 0 {
		return block
	}

	newBlockDataData := make([][]byte, 0, len(block.Data.Data))
	for _, envBytes := range block.Data.Data {
		...
		// 对于系统链码提案的交易，不进行过滤，即先判断交易是否来自系统链码；若是，则加入newBlockDataData
		chdr, err := utils.UnmarshalChannelHeader(payload.Header.ChannelHeader)
		if err != nil {
			continue
		}
		ccs := &peer.ChaincodeSpec{}
		err = pb.Unmarshal(chdr.Extension, ccs)
		ccName := ccs.ChaincodeId.Name
		if ccName == "cscc" || ccName == "lscc" || ccName == "escc" || ccName == "vscc" || ccName == "qscc" ||
			ccName == "rbccustomer" || ccName == "rbcapproval" || ccName == "rbcmodel" {
			newBlockDataData = append(newBlockDataData, envBytes)
			continue
		}
		// 对于非系统链码提案的交易，进行过滤，过滤方法为：在certsList列表中寻找是否含有此交易的证书，若有，则直接将envBytes原封不动加入newBlockDataData；
		// 若没有，则envBytes不加入newBlockDataData
		...
		err = pb.Unmarshal(sHdr.Creator, serializedIdentity)
		if err != nil {
			continue
		}
		certFromTx := serializedIdentity.IdBytes
		for _, cert := range certsList {
			if len(certFromTx) != len([]byte(cert)) {
				// 从交易反序列化出来的证书会多一个LF(NL line feed, new line，换行键)字节，所以比较的时候要去掉最后一个字节
				if bytes.Equal(certFromTx[:len(certFromTx)-1], []byte(cert)) {
					newBlockDataData = append(newBlockDataData, envBytes)
					break
				}
			} else {
				// 证书的字节数组长度相等的时候，直接比较证书的字符串
				if cert == string(certFromTx) {
					newBlockDataData = append(newBlockDataData, envBytes)
					break
				}
			}
	...
	block.Data.Data = newBlockDataData
	return block
}

// Handle light-weight state request message, validate batch size, read current leader state to
// obtain required blocks, build response message and send it back
func (s *GossipStateProviderImpl) handleLightWeightStateRequest(msg proto.ReceivedMessage) {
	if msg == nil {
		return
	}
	request := msg.GetGossipMessage().GetLightweightRequest()
	log.Logger.Warnf("handleLightWeightStateRequest connectionInfo : %s msg : %s", msg.GetConnectionInfo(), msg)
	//batchSize := request.EndSeqNum - request.StartSeqNum
	//if batchSize > defAntiEntropyBatchSize {
	//	log.Logger.Errorf("Requesting blocks batchSize size (%d) greater than configured allowed"+
	...
	// 首先根据request.members从世界状态数据库中获取每个member对应的证书
	MemberCertData.Lock()
	defer MemberCertData.Unlock()
	var certificates []string
	var cert string
	for _, member := range request.Members {
		if v, ok := MemberCertData.MapBuf[member]; ok {
			cert = v
		} else {
			cert, err = util2.GetCertFromDb(member, s.chainID)
			if err != nil || cert == "" {
				log.Logger.Errorf("can't get cert of %v from stateDb, so skip this member in light-weight ledger", member)
				continue
			}
			MemberCertData.MapBuf[member] = cert
		}
		certificates = append(certificates, cert)
	}
	// 若连一个member的证书都找不到，则不回复
	if len(certificates) == 0 {
		log.Logger.Error("no known member in request.members, ignoring request...")
		return
	}
	// 若至少有一个member的证书可从链上获取，则开始遍历每个block并进行过滤操作
	for seqNum := request.StartSeqNum; seqNum <= endSeqNum; seqNum++ {
		log.Logger.Debug("Reading block ", seqNum, " from the committer service")
		blocks := s.committer.GetBlocks([]uint64{seqNum})
		if len(blocks) == 0 {
		...
		// 从block中提取来自指定会员提案的交易
		newBlock := filterBlock(blocks[0], certificates)
		blockBytes, err := pb.Marshal(newBlock)
		if err != nil {
			log.Logger.Errorf("Could not marshal block: %s", err)
		}
	...
	log.Logger.Warnf("handleLightWeightStateRequest response msg connectionInfo : %s", msg.GetConnectionInfo())
}
...
func (s *GossipStateProviderImpl) antiEntropy() {
	...
	antiEntropyInterval := util2.GetDurationOrDefault("core.peer.gossip.antiEntropyInterval", defAntiEntropyInterval)
	for {
		select {
		case <-s.stopCh:
			s.stopCh <- struct{}{}
...
func (s *GossipStateProviderImpl) requestBlocksInRange(start uint64, end uint64) {
	...
	for prev := start; prev <= end; {
		...
		for !responseReceived {
			...
			antiEntropyStateResponseTimeout := util2.GetDurationOrDefault("core.peer.gossip.antiEntropyStateResponseTimeout", defAntiEntropyStateResponseTimeout)
...
func (s *GossipStateProviderImpl) commitBlock(block *common.Block) error {
	...
	b, err := nodeMetastate.Bytes()
	if err == nil {
		s.gossip.UpdateChannelMetadata(b, common3.ChainID(s.chainID))
		log.Logger.Warnf("Update chain metadata. Ledger height: %d", nodeMetastate.LedgerHeight)
	} else {
		log.Logger.Errorf("Unable to serialize node meta nodeMetastate, error = %s", err)
	}
