...
import (
	...
	"github.com/rongzer/blockchain/peer/gossip/state"
...
func newPeerNodeWithGossip(config *gossip.Config, committer committer.Committer, acceptor peerIdentityAcceptor, g gossip.Gossip) *peerNode {
	...
	state.MemberCertData.Lock()
	defer state.MemberCertData.Unlock()
	state.MemberCertData.MapBuf["www"] = "www"

	sp := state.NewGossipStateProvider("testchainid", g, committer, cs)
	if sp == nil {
		return nil
	}
...
func TestLightGossipDirectMsg(t *testing.T) {
	mc := &mockCommitter{}
	...
	p := newLightNodeWithGossip(newGossipConfig(3), mc, noopPeerIdentityAcceptor, g, "www")
	defer p.shutdown()
...
func TestLightGossipReception(t *testing.T) {
	...
	mc.On("LedgerHeight", mock.Anything).Return(uint64(1), nil)
	p := newLightNodeWithGossip(newGossipConfig(5), mc, noopPeerIdentityAcceptor, g, "www")
	defer p.shutdown()
	select {
	case <-receivedChan:
...
func TestLightGossipAccessControl(t *testing.T) {
	// 读取core.yaml配置文件
	testhelper.SetupCoreYAMLConfig()
	testPath := "/tmp/rongzer/test/gossiplight"
	...
	for i := 0; i < bootstrapSetSize; i++ {
		...
		bootstrapSet = append(bootstrapSet, newLightNode(newGossipConfig(i+10), commit, blockPullPolicy, "www"))
	}
	...
	for i := 0; i < standardPeerSetSize; i++ {
		...
		peersSet = append(peersSet, newLightNode(newGossipConfig(15+i, 10), commit, blockPullPolicy, "www"))
	}
...
func TestLightGossip_SendingManyMessages(t *testing.T) {
	// 读取core.yaml配置文件
	testhelper.SetupCoreYAMLConfig()
	testPath := "/tmp/rongzer/test/gossiplight1"
	viper.Set("peer.fileSystemPath", testPath)
	// 账本管理器初始化
	func() {
		os.RemoveAll(testPath)
	...
	for i := 0; i < bootstrapSetSize; i++ {
		commit := newCommitter(i)
		bootstrapSet = append(bootstrapSet, newPeerNode(newGossipConfig(i+20), commit, noopPeerIdentityAcceptor))
	}
	...
	for i := 0; i < standartPeersSize; i++ {
		commit := newCommitter(bootstrapSetSize + i)
		peersSet = append(peersSet, newLightNode(newGossipConfig(30+i, 20), commit, noopPeerIdentityAcceptor, "www"))
	}
...
func TestLightGossip_LightWeightRequest(t *testing.T) {
	// 读取core.yaml配置文件
	testhelper.SetupCoreYAMLConfig()
	testPath := "/tmp/rongzer/test/gossiplight2"
	viper.Set("peer.fileSystemPath", testPath)
	// 账本管理器初始化
	func() {
		os.RemoveAll(testPath)
	...
	bootPeer := newPeerNode(newGossipConfig(40), newCommitter(0), noopPeerIdentityAcceptor)
	...
	// light节点去同步bootPeer
	peer := newLightNode(newGossipConfig(41, 40), newCommitter(1), noopPeerIdentityAcceptor, "www")
	defer peer.shutdown()
	...
	case <-messageCh:
		{
			...
			waitUntilTrueOrTimeout(t, func() bool {
				if len(peer.g.PeersOfChannel(common.ChainID("testchainid"))) != 1 {
					log.Logger.Info("Peer discovery has not finished yet")
					return false
				}
				log.Logger.Info("All peer discovered each other!!!")
				return true
			...
			log.Logger.Info("Waiting for all blocks to arrive.")
			waitUntilTrueOrTimeout(t, func() bool {
				log.Logger.Debug("Trying to see all peers get all blocks")
...
func TestLightGossip_FailedLightWeightRequest(t *testing.T) {
	// 读取core.yaml配置文件
	testhelper.SetupCoreYAMLConfig()
	testPath := "/tmp/rongzer/test/gossiplight3"
	viper.Set("peer.fileSystemPath", testPath)
	// 账本管理器初始化
	func() {
		os.RemoveAll(testPath)
	...
	// light节点去同步bootPeer
	peer := newLightNode(newGossipConfig(51, 50), newCommitter(1), noopPeerIdentityAcceptor, "zzz")
	defer peer.shutdown()
