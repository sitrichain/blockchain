...
func NewMessageStore(pol common2.MessageReplacingPolicy, trigger invalidationTrigger) MessageStore {
...
func NewMessageStoreExpirable(pol common2.MessageReplacingPolicy, trigger invalidationTrigger, msgTTL time.Duration, externalLock func(), externalUnlock func(), externalExpire func(interface{})) MessageStore {
...
func newMsgStore(pol common2.MessageReplacingPolicy, trigger invalidationTrigger) *messageStoreImpl {
...
type messageStoreImpl struct {
	pol               common2.MessageReplacingPolicy
	lock              sync.RWMutex
	messages          []*msg
...
func (s *messageStoreImpl) Add(message interface{}) bool {
	...
	for i := 0; i < n; i++ {
		...
		case common2.MessageInvalidated:
			return false
		case common2.MessageInvalidates:
			s.invTrigger(m.data)
...
func (s *messageStoreImpl) CheckValid(message interface{}) bool {
	...
	for _, m := range s.messages {
		if s.pol(message, m.data) == common2.MessageInvalidated {
			return false
		}
	}
	return true
