...
import (
	...
	algo2 "github.com/rongzer/blockchain/peer/gossip/gossip/algo"
...
func (df DigestFilter) byContext() algo2.DigestFilter {
...
func NewPullMediator(config Config, adapter *PullAdapter) Mediator {
	...
	p.engine = algo2.NewPullEngineWithFilter(p, config.PullInterval, digFilter.byContext())
	return p
}
...
func (p *pullMediatorImpl) HandleMessage(m proto.ReceivedMessage) {
	...
	var itemIDs []string
	var items []*proto.SignedGossipMessage
...
func (p *pullMediatorImpl) Hello(dest string, nonce uint64) {
	...
	log.Logger.Debug("Sending", p.config.MsgType, "hello to", dest)
...
func (p *pullMediatorImpl) SendDigest(digest []string, nonce uint64, context interface{}) {
	...
	log.Logger.Debug("Sending", p.config.MsgType, "digest:", digMsg.GetDataDig().Digests, "to", remotePeer)
...
func (p *pullMediatorImpl) SendReq(dest string, items []string, nonce uint64) {
	...
	log.Logger.Debug("Sending", req, "to", dest)
...
func (p *pullMediatorImpl) SendRes(items []string, context interface{}, nonce uint64) {
	var items2return []*proto.Envelope
	p.RLock()
	defer p.RUnlock()
	...
	remotePeer := context.(proto.ReceivedMessage).GetConnectionInfo()
	log.Logger.Debug("Sending", len(returnedUpdate.GetDataUpdate().Data), p.config.MsgType, "items to", remotePeer)
	context.(proto.ReceivedMessage).Respond(returnedUpdate)
}

func (p *pullMediatorImpl) peersWithEndpoints(endpoints ...string) []*comm2.RemotePeer {
...
func SelectEndpoints(k int, peerPool []discovery2.NetworkMember) []*comm2.RemotePeer {
	if len(peerPool) < k {
		k = len(peerPool)
	}

	indices := util2.GetRandomIndices(k, len(peerPool)-1)
	endpoints := make([]*comm2.RemotePeer, len(indices))
	for i, j := range indices {
		endpoints[i] = &comm2.RemotePeer{Endpoint: peerPool[j].PreferredEndpoint(), PKIID: peerPool[j].PKIid}
	}
	return endpoints
}
