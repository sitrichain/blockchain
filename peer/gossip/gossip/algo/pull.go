...
type PullEngine struct {
	...
	lock               sync.Mutex
	outgoingNONCES     *util2.Set
	incomingNONCES     *util2.Set
...
func NewPullEngineWithFilter(participant PullAdapter, sleepTime time.Duration, df DigestFilter) *PullEngine {
	engine := &PullEngine{
		...
		incomingNONCES:     util2.NewSet(),
		outgoingNONCES:     util2.NewSet(),
		digFilter:          df,
		digestWaitTime:     util2.GetDurationOrDefault("core.peer.gossip.digestWaitTime", defDigestWaitTime),
		requestWaitTime:    util2.GetDurationOrDefault("core.peer.gossip.requestWaitTime", defRequestWaitTime),
		responseWaitTime:   util2.GetDurationOrDefault("core.peer.gossip.responseWaitTime", defResponseWaitTime),
	}
...
func (engine *PullEngine) processIncomingDigests() {
	...
	for n, sources := range engine.item2owners {
		...
		source := sources[util2.RandomInt(len(sources))]
...
func (engine *PullEngine) OnDigest(digest []string, nonce uint64, _ interface{}) {
