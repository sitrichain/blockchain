...
func (ce *configEventer) ProcessConfigUpdate(config Config) {
	log.Logger.Debugf("Processing new config for channel %s", config.ChainID())
	orgMap := cloneOrgConfig(config.Organizations())
	if ce.lastConfig != nil && reflect.DeepEqual(ce.lastConfig.orgMap, orgMap) {
		log.Logger.Debugf("Ignoring new config for channel %s because it contained no anchor peer updates", config.ChainID())
		return
	}
	...
	log.Logger.Debugf("Calling out because config was updated for channel %s", config.ChainID())
