...
func (vscc *ValidatorOneValidSignature) Init(_ shim.ChaincodeStubInterface) pb.Response {
...
func (vscc *ValidatorOneValidSignature) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	log.Logger.Debugf("VSCC invoked")

	// get the envelope...
	env, err := utils.GetEnvelopeFromBlock(args[1])
	if err != nil {
		log.Logger.Errorf("VSCC error: GetEnvelope failed, err %s", err)
	...
	if err != nil {
		log.Logger.Errorf("VSCC error: GetPayload failed, err %s", err)
		return shim.Error(err.Error())
	}
	...
	if err != nil {
		log.Logger.Errorf("VSCC error: pProvider.NewPolicy failed, err %s", err)
		return shim.Error(err.Error())
	}

	// validate the payload type
	if common.HeaderType(chdr.Type) != common.HeaderType_ENDORSER_TRANSACTION {
		log.Logger.Errorf("Only Endorser Transactions are supported, provided type %d", chdr.Type)
	...
	if err != nil {
		log.Logger.Errorf("VSCC error: GetTransaction failed, err %s", err)
		return shim.Error(err.Error())
	}

	// loop through each of the actions within
	for _, act := range tx.Actions {
		p, err := utils.GetChaincodeActionPayload(act.Payload)
		if err != nil {
			log.Logger.Errorf("VSCC error: GetChaincodeActionPayload failed, err %s", err)
			return shim.Error(err.Error())
		}

		signatureSet, err := vscc.deduplicateIdentity(p)
		if err != nil {
			return shim.Error(err.Error())
		}
		...
		if err != nil {
			log.Logger.Warnf("Endorsement policy failure for transaction txid=%s, err: %s", chdr.GetTxId(), err.Error())
			if len(signatureSet) < len(p.Action.Endorsements) {
		...
		if err != nil {
			log.Logger.Errorf("VSCC error: GetChaincodeHeaderExtension failed, err %s", err)
			return shim.Error(err.Error())
		}

		// do some extra validation that is specific to lscc
		if hdrExt.ChaincodeId.Name == "lscc" {
			log.Logger.Debugf("VSCC info: doing special validation for LSCC")

			err = vscc.ValidateLSCCInvocation(stub, chdr.ChannelId, env, p, payl)
			if err != nil {
				log.Logger.Errorf("VSCC error: ValidateLSCCInvocation failed, err %s", err)
				return shim.Error(err.Error())
			}
		}
	}

	log.Logger.Debugf("VSCC exists successfully")
...
func (vscc *ValidatorOneValidSignature) checkInstantiationPolicy(chainName string, env *common.Envelope, instantiationPolicy []byte, payl *common.Payload) error {
	...
	log.Logger.Debugf("VSCC info: checkInstantiationPolicy starts, policy is %#v", instPol)
...
func (vscc *ValidatorOneValidSignature) ValidateLSCCInvocation(_ shim.ChaincodeStubInterface, chid string, env *common.Envelope, cap *pb.ChaincodeActionPayload, payl *common.Payload) error {
	cpp, err := utils.GetChaincodeProposalPayload(cap.ChaincodeProposalPayload)
	if err != nil {
		log.Logger.Errorf("VSCC error: GetChaincodeProposalPayload failed, err %s", err)
		return err
	}

	cis := &pb.ChaincodeInvocationSpec{}
	if err := cis.Unmarshal(cpp.Input); err != nil {
		log.Logger.Errorf("VSCC error: Unmarshal ChaincodeInvocationSpec failed, err %s", err)
		return err
	}
	...
	if cis.ChaincodeSpec == nil ||
		...
		log.Logger.Errorf("VSCC error: committing invalid vscc invocation")
	...
	log.Logger.Debugf("VSCC info: ValidateLSCCInvocation acting on %s %#v", lsccFunc, lsccArgs)

	switch lsccFunc {
	case lscc.UPGRADE, lscc.DEPLOY:
		log.Logger.Debugf("VSCC info: validating invocation of lscc function %s on arguments %#v", lsccFunc, lsccArgs)
		...
		for _, ns := range txRWSet.NsRwSets {
			log.Logger.Debugf("Namespace %s", ns.NameSpace)
		...
		log.Logger.Debugf("Validating %s for cc %s version %s", lsccFunc, cdRWSet.Name, cdRWSet.Version)
...
func (vscc *ValidatorOneValidSignature) deduplicateIdentity(cap *pb.ChaincodeActionPayload) ([]*common.SignedData, error) {
	...
	for _, endorsement := range cap.Action.Endorsements {
		...
		if err := serializedIdentity.Unmarshal(endorsement.Endorser); err != nil {
			log.Logger.Errorf("Unmarshal endorser error: %s", err)
		...
		if _, ok := signatureMap[identity]; ok {
			...
			log.Logger.Warnf("Ignoring duplicated identity, Mspid: %s, pem:\n%s", serializedIdentity.Mspid, serializedIdentity.IdBytes)
			continue
		}
	...
	log.Logger.Debugf("Signature set is of size %d out of %d endorsement(s)", len(signatureSet), len(cap.Action.Endorsements))
