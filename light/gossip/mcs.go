...
import (
	...
	pcommon "github.com/rongzer/blockchain/protos/common"
...
func (s *mspMessageCryptoService) GetPKIidOfCert(peerIdentity api.PeerIdentityType) common.PKIidType {
	...
	if err != nil {
		log.Logger.Errorf("Failed getting validated identity from peer identity [% x]: [%s]", peerIdentity, err)
	...
	if err != nil {
		log.Logger.Errorf("Failed computing digest of serialized identity [% x]: [%s]", peerIdentity, err)
...
func (s *mspMessageCryptoService) VerifyBlock(chainID common.ChainID, seqNum uint64, signedBlock []byte) error {
	...
	// 若该区块为"轻量区块"且其内交易数为0，则不进行后续验证工作
	...
	// 若该区块为"轻量区块"且其内交易数不为0，则不进行区块头验证
	...
	log.Logger.Debugf("Got policy manager for channel [%s] with flag [%s]", channelID, ok)
	...
	log.Logger.Debugf("Got block validation policy for channel [%s] with flag [%s]", channelID, ok)

	// - Prepare SignedData
	var signatureSet []*pcommon.SignedData
...
func (s *mspMessageCryptoService) Verify(peerIdentity api.PeerIdentityType, signature, message []byte) error {
	...
	if err != nil {
		log.Logger.Errorf("Failed getting validated identity from peer identity [%s]", err)
...
func (s *mspMessageCryptoService) VerifyByChannel(chainID common.ChainID, peerIdentity api.PeerIdentityType, signature, message []byte) error {
	...
	log.Logger.Debugf("Got policy manager for channel [%s] with flag [%s]", string(chainID), flag)

	// Get channel reader policy
	policy, flag := cpm.GetPolicy(policies.ChannelApplicationReaders)
	log.Logger.Debugf("Got reader policy for channel [%s] with flag [%s]", string(chainID), flag)
...
func (s *mspMessageCryptoService) getValidatedIdentity(peerIdentity api.PeerIdentityType) (msp.Identity, common.ChainID, error) {
	...
	// of the signature.
	identity, err := s.deserializer.GetLocalDeserializer().DeserializeIdentity(peerIdentity)
	if err == nil {
	...
	for chainID, mspManager := range s.deserializer.GetChannelDeserializers() {
		...
		if err := identity.Validate(); err != nil {
			log.Logger.Debugf("Failed validating identity [% x] on [%s]: [%s]", peerIdentity, chainID, err)
			continue
		}

		log.Logger.Debugf("Validation succeeded [% x] on [%s]", peerIdentity, chainID)
