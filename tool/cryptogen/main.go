...
import (
	...
	"github.com/rongzer/blockchain/tool/cryptogen/msp"
...
//command line flags
var (
	app = kingpin.New("cryptogen", "Utility for generating Rongzer Blockchain key material")
	...
	etype = gen.Flag("type", "The output directory in which to place artifacts").Default("ecdsa").String()
...
func extend() {
	...
	for _, orgSpec := range config.PeerOrgs {
		...
		extendPeerOrg(*inputDir, orgSpec)
	}
...
func extendPeerOrg(baseDir string, orgSpec OrgSpec) {
	orgName := orgSpec.Domain
	orgDir := filepath.Join(baseDir, "peerOrganizations", orgName)
	...
	signCA := getCA(caDir, orgSpec.CA.CommonName)
	tlsCA := getCA(tlscaDir, "tls"+orgSpec.CA.CommonName)
...
func getCA(caDir string, name string) *ca.CA {
...
func generateSm2() {
	...
	for _, orgSpec := range config.PeerOrgs {
		...
		generatePeerOrgSm2(*outputDir, orgSpec)
	}
	...
	for _, orgSpec := range config.OrdererOrgs {
		...
		generateOrdererOrgSm2(*outputDir, orgSpec)
	}
}

func generatePeerOrgSm2(baseDir string, orgSpec OrgSpec) {
	...
	adminCertsDir := filepath.Join(mspDir, "admincerts")
	// generate signing CA
	signCA, err := ca.NewCASm2(caDir, orgName, orgSpec.CA.CommonName)
	if err != nil {
		fmt.Printf("Error generating signCA for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}
	// generate TLS CA
	tlsCA, err := ca.NewCASm2(tlsCADir, orgName, "tls"+orgSpec.CA.CommonName)
	if err != nil {
		fmt.Printf("Error generating tlsCA for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}
	...
	generateSm2Nodes(peersDir, orgSpec.Specs, signCA, tlsCA)
	...
	users = append(users, adminUser)
	generateSm2Nodes(usersDir, users, signCA, tlsCA)
...
func generateOrdererOrgSm2(baseDir string, orgSpec OrgSpec) {
	...
	generateSm2Nodes(orderersDir, orgSpec.Specs, signCA, tlsCA)
...
func generateSm2Nodes(baseDir string, nodes []NodeSpec, signCA *ca.CA, tlsCA *ca.CA) {

	for _, node := range nodes {
		nodeDir := filepath.Join(baseDir, node.CommonName)
		err := msp.GenerateLocalMSPSm2(nodeDir, node.CommonName, node.SANS, signCA, tlsCA)
		if err != nil {
			fmt.Printf("Error generating local MSP for %s:\n%v\n", node, err)
			os.Exit(1)
		}
