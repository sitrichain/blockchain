...
func newBrokerConfig(tlsConfig localconfig.TLS, retryOptions localconfig.Retry, kafkaVersion sarama.KafkaVersion) *sarama.Config {
	...
	// Allows us to retrieve errors that occur when consuming a chain
	...
	if brokerConfig.Net.TLS.Enable {
		...
		if err != nil {
			log.Logger.Panic("Unable to decode public/private key pair:", err)
		}
		// create root CA pool
		rootCAs := x509.NewCertPool()
		for _, certificate := range tlsConfig.RootCAs {
			if !rootCAs.AppendCertsFromPEM([]byte(certificate)) {
				log.Logger.Panic("Unable to parse the root certificate authority certificates (Kafka.Tls.RootCAs)")
			}
		}
	...
	brokerConfig.Producer.Partitioner = newStaticPartitioner(0)
