...
func (ce *ConfigUpdateEnvelope) AsSignedData() ([]*SignedData, error) {
	...
	for i, configSig := range ce.Signatures {
		...
		if err := sigHeader.Unmarshal(configSig.SignatureHeader); err != nil {
			return nil, err
		}
...
func (env *Envelope) AsSignedData() ([]*SignedData, error) {
	...
	payload := &Payload{}
	if err := payload.Unmarshal(env.Payload); err != nil {
		return nil, err
	}
	...
	if err := shdr.Unmarshal(payload.Header.SignatureHeader); err != nil {
