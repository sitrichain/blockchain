...
import (
	...
	configmsp "github.com/rongzer/blockchain/common/config/msp"
...
func (ct *compositeTemplate) Envelope(chainID string) (*cb.ConfigUpdateEnvelope, error) {
	...
	for i := range ct.templates {
		...
		conf, err := UnmarshalConfigUpdate(configEnv.ConfigUpdate)
		if err != nil {
			return nil, err
		}
		err = copyGroup(conf.WriteSet, channel)
		if err != nil {
			return nil, err
		}
	}
	...
	marshaledConfig, err := cu.Marshal()
	if err != nil {
		return nil, err
	}
...
func (mpst *modPolicySettingTemplate) Envelope(channelID string) (*cb.ConfigUpdateEnvelope, error) {
	...
	conf, err := UnmarshalConfigUpdate(configUpdateEnv.ConfigUpdate)
	if err != nil {
		return nil, err
	}

	setGroupModPolicies(mpst.modPolicy, conf.WriteSet)
	configUpdateEnv.ConfigUpdate = utils.MarshalOrPanic(conf)
...
func MakeChainCreationTransaction(channelID string, consortium string, signer msp.SigningIdentity, orgs ...string) (*cb.Envelope, error) {
	...
	payloadChannelHeader := utils.MakeChannelHeader(cb.HeaderType_CONFIG_UPDATE, channelID)
