...
type ConnectionMapper interface {
	Lookup(endPoint string) (*grpc.ClientConn, bool)
	Put(endPoint string, conn *grpc.ClientConn)
...
func (c *ConnectionStore) Disconnect(endPoint string) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *ConnectionStore) Connection(endpoint string) (*grpc.ClientConn, error) {
	c.lock.RLock()
...
func (c *ConnectionStore) connect(endpoint string) (*grpc.ClientConn, error) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (cmg *connMapperReporter) Put(endPoint string, conn *grpc.ClientConn) {
	cmg.ConnectionMapper.Put(endPoint, conn)
}

func (cmg *connMapperReporter) Remove(endPoint string) {
	cmg.ConnectionMapper.Remove(endPoint)
}
