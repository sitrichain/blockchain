...
func (c *Comm) TargetChannel(message proto.Message) string {
...
// GetSourceNodeInfo extracts the SourceNodeInfo from the given proto.Message.
// Returns an empty string on failure.
func (c *Comm) GetSourceNodeInfo(message proto.Message) *RemoteNode {
	switch req := message.(type) {
	case *orderer.ConsensusRequest:
		source := req.SourceNodeInfo
		return &RemoteNode{ID: source.Id, Endpoint: source.EndPoint}
	case *orderer.SubmitRequest:
		source := req.SourceNodeInfo
		return &RemoteNode{ID: source.Id, Endpoint: source.EndPoint}
	default:
...
func (c *Comm) requestContext(ctx context.Context, msg proto.Message) (*requestContext, error) {
	channel := c.TargetChannel(msg)
	if channel == "" {
		return nil, errors.Errorf("badly formatted message, cannot extract channel")
	}
	node := c.GetSourceNodeInfo(msg)
	// 若能获取到msg的来源节点信息，则更新本地保存的stub
	if node != nil {
		c.Lock.Lock()
		memberMapping := c.getOrCreateMapping(channel)
		c.updateStubInMapping(channel, memberMapping, node)
		c.Lock.Unlock()
	}
...
func (c *Comm) Configure(channel string, newNodes []*RemoteNode) {
	...
	beforeConfigChange := c.endPointsInUse()
...
func (c *Comm) Shutdown() {
	...
	for _, members := range c.Chan2Members {
		for _, member := range members {
			c.Connections.Disconnect(member.Endpoint)
		}
	}
}
...
func (c *Comm) cleanUnusedConnections(endPointsBeforeConfig StringSet) {
	// Scan all nodes after the reconfiguration
	endPointsAfterConfig := c.endPointsInUse()
	// Filter out the certificates that remained after the reconfiguration
	endPointsBeforeConfig.subtract(endPointsAfterConfig)
	// Close the connections to all these nodes as they shouldn't be in use now
	for endPoint := range endPointsBeforeConfig {
		c.Connections.Disconnect(endPoint)
	}
}

// endPointsInUse returns the server certificates that are in use
// represented as strings.
func (c *Comm) endPointsInUse() StringSet {
	inUse := make(StringSet)
	for _, mapping := range c.Chan2Members {
		inUse.union(mapping.EndPoints())
	}
	return inUse
}
...
func (c *Comm) createRemoteContext(stub *Stub, channel string) func() (*RemoteContext, error) {
	return func() (*RemoteContext, error) {
		...
		conn, err := c.Connections.Connection(stub.Endpoint)
		if err != nil {
...
func (stream *Stream) sendMessage(request *orderer.StepRequest) {
	...
	defer func() {
		...
		stream.Logger.Debugf("Send of %s to %s took %v %s", requestAsString(request),
			stream.Endpoint, time.Since(start), result)
	}()
...
func (stream *Stream) Recv() (*orderer.StepResponse, error) {
	...
	defer func() {
		...
		stream.Logger.Debugf("Receive from %s took %v", stream.Endpoint, time.Since(start))
	}()
...
func (stream *Stream) operateWithTimeout(invoke StreamOperation) (*orderer.StepResponse, error) {
	...
	case r := <-responseChan:
		...
		return r.res, r.err
	case <-timer.C:
		stream.Logger.Warningf("Stream %d to %s was forcibly terminated because timeout (%v) expired",
			stream.ID, stream.Endpoint, stream.Timeout)
...
func (rc *RemoteContext) NewStream(timeout time.Duration) (*Stream, error) {
	...
	abort := func() {
		...
		rc.Logger.Debugf("Stream %d to %s is aborted", streamID, rc.endpoint)
	...
	rc.Logger.Infof("Created new stream to %s with ID of %d and buffer size of %d",
		rc.endpoint, streamID, cap(s.sendBuff))

	rc.streamsByID.Store(streamID, s)

	go func() {
		rc.workerCountReporter.increment()
		s.serviceStream()
		rc.workerCountReporter.decrement()
	}()
...
func (wcr *workerCountReporter) increment() {
	atomic.AddUint32(&wcr.workerCount, 1)
}

func (wcr *workerCountReporter) decrement() {
	...
	atomic.AddUint32(&wcr.workerCount, ^uint32(0))
}
