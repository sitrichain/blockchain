...
func (og *OrganizationGroup) NewGroup(_ string) (ValueProposer, error) {
...
func NewOrganizationConfig(og *OrganizationGroup) *OrganizationConfig {
	...
	if err != nil {
		log.Logger.Panicf("Programming error: %s", err)
	}
	return oc
}

// Validate returns whether the configuration is valid
func (oc *OrganizationConfig) Validate(tx interface{}, _ map[string]ValueProposer) error {
...
func (oc *OrganizationConfig) validateMSP(tx interface{}) error {
	...
	log.Logger.Debugf("Setting up MSP for org %s", oc.organizationGroup.name)
