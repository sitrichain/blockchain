...
func (p *Proposer) BeginValueProposals(tx interface{}, groups []string) (ValueDeserializer, []ValueProposer, error) {
	...
	if _, ok := p.pending[tx]; ok {
		log.Logger.Panicf("Duplicated BeginValueProposals without Rollback or Commit")
	}
...
func (p *Proposer) PreCommit(tx interface{}) error {
	...
	if !ok {
		log.Logger.Panicf("Serious Programming Error: attempted to pre-commit tx which had not been begun")
	}
...
func (p *Proposer) CommitProposals(tx interface{}) {
	...
	pending, ok := p.pending[tx]
	if !ok {
		log.Logger.Panicf("Serious Programming Error: attempted to commit tx which had not been begun")
	}
	p.current = pending
