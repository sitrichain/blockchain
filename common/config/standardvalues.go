...
func NewStandardValues(protosStructs ...interface{}) (*standardValues, error) {
	...
	for _, protosStruct := range protosStructs {
		log.Logger.Debugf("Initializing protos for %T\n", protosStruct)
...
func (sv *standardValues) initializeProtosStruct(objValue reflect.Value) error {
	...
	for i := 0; i < numFields; i++ {
		...
		log.Logger.Debugf("Processing field: %s\n", structField.Name)
		...
		msg, ok := objValue.Elem().Field(i).Interface().(proto.Message)
		if !ok {
		...
		sv.lookup[structField.Name] = msg
	}
