...
func CreateDirIfMissing(dirPath string) (bool, error) {
	...
	log.Logger.Debugf("CreateDirIfMissing [%s]", dirPath)
	logDirStatus("Before creating dir", dirPath)
	err := os.MkdirAll(path.Dir(dirPath), 0755)
	if err != nil {
		log.Logger.Debugf("Error while creating dir [%s]", dirPath)
		return false, err
	}
...
func DirEmpty(dirPath string) (bool, error) {
	...
	if err != nil {
		log.Logger.Debugf("Error while opening dir [%s]: %s", dirPath, err)
		return false, err
	}
	defer f.Close()
...
func logDirStatus(msg string, dirPath string) {
	...
	if err != nil {
		log.Logger.Errorf("Error while checking for dir existence")
	}
	if exists {
		log.Logger.Debugf("%s - [%s] exists", msg, dirPath)
	} else {
		log.Logger.Debugf("%s - [%s] does not exist", msg, dirPath)
	}
}
