...
func (itr *blocksItr) waitForBlock(blockNum uint64) uint64 {
	...
	for itr.mgr.cpInfo.lastBlockNumber < blockNum && !itr.shouldClose() {
		log.Logger.Debugf("Going to wait for newer blocks. maxAvailaBlockNumber=[%d], waitForBlockNum=[%d]",
			itr.mgr.cpInfo.lastBlockNumber, blockNum)
		itr.mgr.cpInfoCond.Wait()
		log.Logger.Debugf("Came out of wait. maxAvailaBlockNumber=[%d]", itr.mgr.cpInfo.lastBlockNumber)
	}
