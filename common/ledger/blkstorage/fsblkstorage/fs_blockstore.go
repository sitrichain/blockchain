...
func newFsBlockStore(id string, conf *Conf, indexConfig *blkstorage.IndexConfig,
	dbHandle util.DBHandle) *fsBlockStore {
...
func (store *fsBlockStore) RetrieveErrorTxChain(txID string) (*peer.ErrorTxChain, error) {
	return store.fileMgr.retrieveErrorTxChain(txID)
}

func (store *fsBlockStore) RetrieveLatestErrorTxChain() (string, error) {
	return store.fileMgr.retrieveLatestErrorTxChain()
}

// Shutdown shuts down the block store
func (store *fsBlockStore) Shutdown() {
	log.Logger.Debugf("closing fs blockStore:%s", store.id)
	store.fileMgr.close()
}

// 获取Attach clob信息
func (store *fsBlockStore) GetAttach(attachKey string) string {
	return store.fileMgr.GetAttach(attachKey)
}
