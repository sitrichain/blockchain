...
func newBlockfileStream(rootDir string, fileNum int, startOffset int64) (*blockfileStream, error) {
	...
	log.Logger.Debugf("newBlockfileStream(): filePath=[%s], startOffset=[%d]", filePath, startOffset)
	var file *os.File
	var err error
...
func (s *blockfileStream) nextBlockBytesAndPlacementInfo() ([]byte, *blockPlacementInfo, error) {
	...
	if s.currentOffset == fileInfo.Size() {
		log.Logger.Debugf("Finished reading file number [%d]", s.fileNum)
		return nil, nil, nil
	}
	...
	log.Logger.Debugf("Remaining bytes=[%d], Going to peek [%d] bytes", remainingBytes, peekBytes)
	...
	if bytesExpected > remainingBytes {
		log.Logger.Debugf("At least [%d] bytes expected. Remaining bytes = [%d]. Returning with error [%s]",
	...
	if _, err = io.ReadAtLeast(s.reader, blockBytes, int(length)); err != nil {
		log.Logger.Debugf("Error while trying to read [%d] bytes from fileNum [%d]: %s", length, s.fileNum, err)
		return nil, nil, err
	}
	...
	log.Logger.Debugf("Returning blockbytes - length=[%d], placementInfo={%s}", len(blockBytes), blockPlacementInfo)
...
func (s *blockStream) nextBlockBytesAndPlacementInfo() ([]byte, *blockPlacementInfo, error) {
	...
	if blockBytes, blockPlacementInfo, err = s.currentFileStream.nextBlockBytesAndPlacementInfo(); err != nil {
		log.Logger.Debugf("current file [%d] length of blockbytes [%d]. Err:%s", s.currentFileNum, len(blockBytes), err)
		return nil, nil, err
	}
	log.Logger.Debugf("blockbytes [%d] read from file [%d]", len(blockBytes), s.currentFileNum)
	if blockBytes == nil && (s.currentFileNum < s.endFileNum || s.endFileNum < 0) {
		log.Logger.Debugf("current file [%d] exhausted. Moving to next file", s.currentFileNum)
