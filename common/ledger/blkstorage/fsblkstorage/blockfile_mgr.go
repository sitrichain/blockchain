...
*/
func newBlockfileMgr(id string, conf *Conf, indexConfig *blkstorage.IndexConfig, indexStore util.DBHandle) *blockfileMgr {
	log.Logger.Debugf("newBlockfileMgr() initializing file-based block storage for ledger: %s ", id)
...
func syncCPInfoFromFS(rootDir string, cpInfo *checkpointInfo) {
	log.Logger.Debugf("Starting checkpoint=%s", cpInfo)
	...
	log.Logger.Debugf("status of file [%s]: exists=[%t], size=[%d]", filePath, exists, size)
	...
	log.Logger.Debugf("Checkpoint after updates by scanning the last file segment:%s", cpInfo)
}
...
func (mgr *blockfileMgr) addBlock(block *common.Block) error {
	...
	blockFLP.beginningOffset = currentOffset
	/* txIdx不需要加偏移，相对于block解压处理
	...
	if err := mgr.index.indexBlock(&blockIdxInfo{
		...
		flp: blockFLP, txOffsets: txOffsets, metadata: block.Metadata, block: block}); err != nil {
		return fmt.Errorf("Save block error because index block error. %v", err)
	}
...
//处理业务模型中的CLOB信息
func (mgr *blockfileMgr) clearAttach(block *common.Block) {
	for tIdx, d := range block.Data.Data {
		if env, err := putil.GetEnvelopeFromBlock(d); err == nil {
			env1 := &common.Envelope{Payload: env.Payload, Signature: env.Signature}
			buf, _ := env1.Marshal()
			block.Data.Data[tIdx] = buf
		}
	}
}

//根据key查询附件信息
func (mgr *blockfileMgr) GetAttach(attachKey string) string {
	//查询附近件的交易
	txID, err := mgr.index.getAttachTxID(attachKey)
	if err != nil {
		log.Logger.Errorf("not find txID err:%s", err)
		return ""
	}

	env, err := mgr.retrieveTransactionByID(txID)
	if err != nil {
		log.Logger.Errorf("not find env err:%s", err)

		return ""
	}

	return env.Attachs[attachKey]
}
...
func (mgr *blockfileMgr) syncIndex() error {
	...
	for {
		...
		blockIdxInfo.flp = &fileLocPointer{fileSuffixNum: blockPlacementInfo.fileNum,
			beginningOffset: int(blockPlacementInfo.blockStartOffset),
		...
		log.Logger.Debugf("syncIndex() indexing block [%d]", blockIdxInfo.blockNum)
...
func (mgr *blockfileMgr) updateCheckpoint(cpInfo *checkpointInfo) {
	...
	mgr.cpInfo = cpInfo
	log.Logger.Debugf("Broadcasting about update checkpointInfo: %s", cpInfo)
...
func (mgr *blockfileMgr) retrieveBlockByHash(blockHash []byte) (*common.Block, error) {
	log.Logger.Debugf("retrieveBlockByHash() - blockHash = [%#v]", blockHash)
...
func (mgr *blockfileMgr) retrieveBlockByNumber(blockNum uint64) (*common.Block, error) {
	log.Logger.Debugf("retrieveBlockByNumber() - blockNum = [%d]", blockNum)
...
func (mgr *blockfileMgr) retrieveBlockByTxID(txID string) (*common.Block, error) {
	log.Logger.Debugf("retrieveBlockByTxID() - txID = [%s]", txID)
...
func (mgr *blockfileMgr) retrieveTxValidationCodeByTxID(txID string) (peer.TxValidationCode, error) {
	log.Logger.Debugf("retrieveTxValidationCodeByTxID() - txID = [%s]", txID)
...
func (mgr *blockfileMgr) retrieveBlockHeaderByNumber(blockNum uint64) (*common.BlockHeader, error) {
	log.Logger.Debugf("retrieveBlockHeaderByNumber() - blockNum = [%d]", blockNum)
	loc, err := mgr.index.getBlockLocByBlockNum(blockNum)
	if err != nil {
		return nil, err
	}
...
func (mgr *blockfileMgr) retrieveTransactionByID(txID string) (*common.Envelope, error) {
	log.Logger.Debugf("retrieveTransactionByID() - txId = [%s]", txID)
...
func (mgr *blockfileMgr) retrieveTransactionByBlockNumTranNum(blockNum uint64, tranNum uint64) (*common.Envelope, error) {
	log.Logger.Debugf("retrieveTransactionByBlockNumTranNum() - blockNum = [%d], tranNum = [%d]", blockNum, tranNum)
...
func (mgr *blockfileMgr) retrieveErrorTxChain(txId string) (*peer.ErrorTxChain, error) {
	return mgr.index.getErrorTxChain(txId)
}

func (mgr *blockfileMgr) retrieveLatestErrorTxChain() (string, error) {
	return mgr.index.getLatestErrorTxChain()
}
...
func (mgr *blockfileMgr) fetchTransactionEnvelope(lp *fileLocPointer) (*common.Envelope, error) {
	log.Logger.Debugf("Entering fetchTransactionEnvelope() %v\n", lp)
	var err error
	var txEnvelopeBytes []byte
	if txEnvelopeBytes, err = mgr.fetchRawBytes(lp); err != nil {
		log.Logger.Errorf("can't find txEnvelope err:%s", err)
		return nil, err
	}
...
func (mgr *blockfileMgr) fetchRawBytes(lp *fileLocPointer) ([]byte, error) {
	stream, err := newBlockfileStream(mgr.rootDir, lp.fileSuffixNum, int64(lp.beginningOffset))
	if err != nil {
		return nil, err
	}
	defer stream.close()
	...
	if err != nil {
		log.Logger.Errorf("find block bytes err:%s", err)

		return nil, err
	}
	blockBytes = Ungzip(blockBytes)
	b := blockBytes
	if lp.offset-lp.beginningOffset+lp.bytesLength < len(blockBytes) && lp.offset-lp.beginningOffset+lp.bytesLength > lp.offset-lp.beginningOffset && lp.offset-lp.beginningOffset > 0 {
		b = blockBytes[lp.offset-lp.beginningOffset : lp.offset-lp.beginningOffset+lp.bytesLength]
	}
...
func (mgr *blockfileMgr) loadCurrentInfo() (*checkpointInfo, error) {
	...
	log.Logger.Debugf("loaded checkpointInfo:%s", i)
	return i, nil
}
...
func scanForLastCompleteBlock(rootDir string, fileNum int, startingOffset int64) (int64, int, error) {
	...
	log.Logger.Debugf("scanForLastCompleteBlock(): last complete block ends at offset=[%d]", blockStream.currentOffset)
