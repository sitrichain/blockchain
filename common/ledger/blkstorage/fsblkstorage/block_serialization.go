...
import (
	...
	ledgerutil "github.com/rongzer/blockchain/common/ledger/util"
...
func serializeBlock(block *common.Block) ([]byte, *serializedBlockInfo, error) {
	...
	return Gzip(buf.Bytes()), info, nil
}

func deserializeBlock(serializedBlockBytes []byte) (*common.Block, error) {
	serializedBlockBytes = Ungzip(serializedBlockBytes)
	block := &common.Block{}
	var err error
...
func addDataBytes(blockData *common.BlockData, buf *proto.Buffer) ([]*txindexInfo, error) {
	...
	for _, txEnvelopeBytes := range blockData.Data {
		...
		//logger.Infof("offset %d,txlen %d, byteLen %d,lp %s,addTx:%s", offset, len(txEnvelopeBytes), len(buf.Bytes())-offset, idxInfo.loc.String(), hex.EncodeToString(txEnvelopeBytes))
...
func addMetadataBytes(blockMetadata *common.BlockMetadata, buf *proto.Buffer) error {
	if blockMetadata == nil {
		return errors.New("empty block metadata")
	}
...
func Gzip(data []byte) []byte {
	//小交易用gzip压缩，大交易压缩性能不够，不压了
	if len(data) > 3*1024*1024+500*1024 {
		return data
	}
