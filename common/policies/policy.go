...
func NewManagerImpl(basePath string, providers map[int32]Provider) *ManagerImpl {
	...
	if ok {
		log.Logger.Panicf("ImplicitMetaPolicy type must be provider by the policy manager")
	}
...
func (rp rejectPolicy) Evaluate([]*cb.SignedData) error {
...
func (pm *ManagerImpl) GetPolicy(id string) (Policy, bool) {
	if id == "" {
		log.Logger.Errorf("Returning dummy reject all policy because no policy ID supplied")
	...
	if strings.HasPrefix(id, PathSeparator) {
		...
		if !strings.HasPrefix(id, pm.fqPrefix) {
			log.Logger.Debugf("Requested policy from root manager with wrong basePath: %s, returning rejectAll", id)
			return rejectPolicy(id), false
		}
	...
	if !ok {
		log.Logger.Debugf("Returning dummy reject all policy because %s could not be found in /%s/%s", id, pm.basePath, relpath)
		return rejectPolicy(relpath), false
	}
	log.Logger.Debugf("Returning policy %s for evaluation", relpath)
	return policy, true
}
...
func (pm *ManagerImpl) BeginPolicyProposals(tx interface{}, groups []string) ([]Proposer, error) {
	...
	if ok {
		log.Logger.Panicf("Serious Programming error: cannot call begin multiply for the same proposal")
	}
...
func (pm *ManagerImpl) PreCommit(interface{}) error {
	return nil
}
...
func (pm *ManagerImpl) CommitProposals(tx interface{}) {
	...
	pendingConfig, ok := pm.pendingConfig[tx]
	if !ok {
		log.Logger.Panicf("Programming error, cannot call begin in the middle of a proposal")
		return
	}

	if pendingConfig == nil {
		log.Logger.Panicf("Programming error, cannot call commit without an existing proposal")
		return
	}
	...
	for managerPath, m := range pendingConfig.managers {
		for _, policyName := range m.PolicyNames() {
			...
			log.Logger.Debugf("In commit adding relative sub-policy %s to %s", fqKey, pm.basePath)
		}
	}
	...
	if pm.parent == nil && pm.basePath == ChannelPrefix && !pm.SuppressSanityLogMessages {
		for _, policyName := range []string{ChannelReaders, ChannelWriters} {
			...
			if !ok {
				log.Logger.Warnf("Current configuration has no policy '%s', this will likely cause problems in production systems", policyName)
			} else {
				log.Logger.Debugf("As expected, current configuration has policy '%s'", policyName)
			}
		}
...
func (pm *ManagerImpl) ProposePolicy(tx interface{}, key string, configPolicy *cb.ConfigPolicy) (proto.Message, error) {
	...
	if !ok {
		log.Logger.Panicf("Serious Programming error: called Propose without Begin")
	}
	...
	if policy.Type == int32(cb.Policy_IMPLICIT_META) {
		...
		provider, ok := pm.providers[policy.Type]
		if !ok {
	...
	log.Logger.Debugf("Proposed new policy %s for %s", key, pm.basePath)
