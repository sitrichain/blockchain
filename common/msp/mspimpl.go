...
import (
	...
	m "github.com/rongzer/blockchain/protos/msp"
)
...
type bccspmsp struct {
	...
	cryptoConfig *m.BlockchainCryptoConfig
}
...
func NewBccspMsp() (MSP, error) {
	log.Logger.Debugf("Creating BCCSP-based MSP instance")
	return &bccspmsp{bccsp: factory.GetDefault()}, nil
}

func (msp *bccspmsp) getCertFromPem(idBytes []byte) (*gm.Certificate, error) {
	if idBytes == nil {
	...
	cert, err := gm.ParseCertificate(pemCert.Bytes)
	if err != nil {
		return nil, fmt.Errorf("getIdentityFromBytes error: failed to parse gm cert, err %s", err)
	}
...
func (msp *bccspmsp) getSigningIdentityFromConf(sidInfo *m.SigningIdentityInfo) (SigningIdentity, error) {
	...
	if err != nil {
		log.Logger.Debugf("Could not find SKI [%s], trying KeyMaterial field: %s\n", hex.EncodeToString(pubKey.SKI()), err)
...
func getSubjectKeyIdentifierFromCert(cert *gm.Certificate) ([]byte, error) {
	var SKI []byte
...
// and false otherwise
func isCACert(cert *gm.Certificate) bool {
...
func (msp *bccspmsp) Setup(conf1 *m.MSPConfig) error {
	...
	// given that it's an msp of type blockchain, extract the MSPConfig instance
	conf := &m.BlockchainMSPConfig{}
	if err := conf.Unmarshal(conf1.Config); err != nil {
		return fmt.Errorf("Failed unmarshalling blockchain msp config, err %s", err)
	}

	// set the name for this msp
	msp.name = conf.Name
	log.Logger.Infof("Setting up MSP instance %s", msp.name)
...
func (msp *bccspmsp) GetDefaultSigningIdentity() (SigningIdentity, error) {
	log.Logger.Debugf("Obtaining default signing identity")
...
func (msp *bccspmsp) Validate(id Identity) error {
	log.Logger.Debugf("MSP %s validating identity", msp.name)
...
func (msp *bccspmsp) deserializeIdentityInternal(serializedIdentity []byte) (Identity, error) {
	...
	cert, err := gm.ParseCertificate(bl.Bytes)
	if err != nil {
...
func (msp *bccspmsp) SatisfiesPrincipal(id Identity, principal *m.MSPPrincipal) error {
	...
	case m.MSPPrincipal_ROLE:
		...
		mspRole := &m.MSPRole{}
		if err := mspRole.Unmarshal(principal.Principal); err != nil {
		...
		case m.MSPRole_MEMBER:
			...
			log.Logger.Debugf("Checking if identity satisfies MEMBER role for %s", msp.name)
			return msp.Validate(id)
		case m.MSPRole_ADMIN:
			log.Logger.Debugf("Checking if identity satisfies ADMIN role for %s", msp.name)
	...
	case m.MSPPrincipal_ORGANIZATION_UNIT:
		...
		if err := OU.Unmarshal(principal.Principal); err != nil {
...
func (msp *bccspmsp) getCertificationChain(id Identity) ([]*gm.Certificate, error) {
	log.Logger.Debugf("MSP %s getting certification chain", msp.name)
...
func (msp *bccspmsp) getCertificationChainForBCCSPIdentity(id *identity) ([]*gm.Certificate, error) {
	if id == nil {
...
func (msp *bccspmsp) getUniqueValidationChain(cert *gm.Certificate, opts gm.VerifyOptions) ([]*gm.Certificate, error) {
...
func (msp *bccspmsp) getValidationChain(cert *gm.Certificate, isIntermediateChain bool) ([]*gm.Certificate, error) {
...
func (msp *bccspmsp) getCertificationChainIdentifierFromChain(chain []*gm.Certificate) ([]byte, error) {
	// Hash the chain
	// Use the hash of the identity's certificate as id in the IdentityIdentifier
	hashOpt, err := bccsp.GetHashOpt(msp.cryptoConfig.IdentityIdentifierHashFunction)
	if err != nil {
...
func (msp *bccspmsp) setupCrypto(conf *m.BlockchainMSPConfig) error {
	msp.cryptoConfig = conf.CryptoConfig
	if msp.cryptoConfig == nil {
		// Move to defaults
		msp.cryptoConfig = &m.BlockchainCryptoConfig{
			SignatureHashFamily:            bccsp.SHA2,
			IdentityIdentifierHashFunction: bccsp.SHA256,
		}
		log.Logger.Debugf("CryptoConfig was nil. Move to defaults.")
	}
	if msp.cryptoConfig.SignatureHashFamily == "" {
		msp.cryptoConfig.SignatureHashFamily = bccsp.SHA2
		log.Logger.Debugf("CryptoConfig.SignatureHashFamily was nil. Move to defaults.")
	}
	if msp.cryptoConfig.IdentityIdentifierHashFunction == "" {
		msp.cryptoConfig.IdentityIdentifierHashFunction = bccsp.SHA256
		log.Logger.Debugf("CryptoConfig.IdentityIdentifierHashFunction was nil. Move to defaults.")
	}

	return nil
}

func (msp *bccspmsp) setupCAs(conf *m.BlockchainMSPConfig) error {
	...
	msp.opts = &gm.VerifyOptions{Roots: gm.NewCertPool(), Intermediates: gm.NewCertPool()}
...
func (msp *bccspmsp) setupAdmins(conf *m.BlockchainMSPConfig) error {
	// make and fill the set of admin certs (if present)
	msp.admins = make([]Identity, len(conf.Admins))
	for i, admCert := range conf.Admins {
		id, _, err := msp.getIdentityFromConf(admCert)
...
func (msp *bccspmsp) setupSigningIdentity(conf *m.BlockchainMSPConfig) error {
...
func (msp *bccspmsp) setupOUs(conf *m.BlockchainMSPConfig) error {
	...
	for _, ou := range conf.OrganizationalUnitIdentifiers {
		...
		var chain []*gm.Certificate
		if root {
			chain = []*gm.Certificate{cert}
		} else {
		...
		for _, id := range msp.ouIdentifiers[ou.OrganizationalUnitIdentifier] {
			if bytes.Equal(id, certifiersIdentitifer) {
				log.Logger.Warnf("Duplicate found in ou identifiers [%s, %v]", ou.OrganizationalUnitIdentifier, id)
				found = true
				break
			}
		}
...
func (msp *bccspmsp) setupTLSCAs(conf *m.BlockchainMSPConfig) error {

	opts := &gm.VerifyOptions{Roots: gm.NewCertPool(), Intermediates: gm.NewCertPool()}

	// Load TLS root and intermediate CA identities
	msp.tlsRootCerts = make([][]byte, len(conf.TlsRootCerts))
	rootCerts := make([]*gm.Certificate, len(conf.TlsRootCerts))
	...
	intermediateCerts := make([]*gm.Certificate, len(conf.TlsIntermediateCerts))
	...
	// ensure that our CAs are properly formed and that they are valid
	for _, cert := range append(append([]*gm.Certificate{}, rootCerts...), intermediateCerts...) {
		if cert == nil {
			continue
		}
...
func (msp *bccspmsp) sanitizeCert(cert *gm.Certificate) (*gm.Certificate, error) {
	if isECDSASignedCert(cert) {
		...
		if cert.IsCA {
			...
			if err != nil {
				log.Logger.Errorf("sanitizeCert is ca err:", err)
			...
			opts := msp.getValidityOptsForCert(cert)
			chain, err := msp.getUniqueValidationChain(cert, opts)
			if err != nil {
				log.Logger.Errorf("sanitizeCert is not ca err:%s", err)

				return nil, err
			}
			parentCert = chain[1]
		}
		log.Logger.Debugf("sanitizeCert begin sanitizeECDSASignedCert")
		// Sanitize
		var err error
		cert, err = sanitizeECDSASignedCert(cert, parentCert)
		if err != nil {
			log.Logger.Errorf("sanitizeCert verify sign err:", err)
...
func (msp *bccspmsp) validateTLSCAIdentity(cert *gm.Certificate, opts *gm.VerifyOptions) error {
	if !cert.IsCA {
		return errors.New("Only CA identities can be validated")
	}
...
func (msp *bccspmsp) validateIdentityAgainstChain(id *identity, validationChain []*gm.Certificate) error {
	return msp.validateCertAgainstChain(id.cert, validationChain)
}

func (msp *bccspmsp) validateCertAgainstChain(cert *gm.Certificate, validationChain []*gm.Certificate) error {
			...
			for _, rc := range crl.TBSCertList.RevokedCertificates {
				if rc.SerialNumber.Cmp(cert.SerialNumber) == 0 {
					...
					if err != nil {
						...
						// candidate CRL - skip
						log.Logger.Warnf("Invalid signature over the identified CRL, error %s", err)
						continue
					}
...
func (msp *bccspmsp) getValidityOptsForCert(cert *gm.Certificate) gm.VerifyOptions {
...
func (msp *bccspmsp) getValidityOptsForTLSCert() gm.VerifyOptions {
	// First copy the opts to override the CurrentTime field
	// in order to make the certificate passing the expiration test
	// independently from the real local current time.
	// This is a temporary workaround for FAB-3678
