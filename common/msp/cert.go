...
func isECDSASignedCert(cert *gm.Certificate) bool {
	return cert.SignatureAlgorithm == gm.ECDSAWithSHA1 ||
		cert.SignatureAlgorithm == gm.ECDSAWithSHA256 ||
		cert.SignatureAlgorithm == gm.ECDSAWithSHA384 ||
		cert.SignatureAlgorithm == gm.ECDSAWithSHA512
}
...
func sanitizeECDSASignedCert(cert *gm.Certificate, parentCert *gm.Certificate) (*gm.Certificate, error) {
	if cert == nil {
	...
	return gm.ParseCertificate(newRaw)
}
