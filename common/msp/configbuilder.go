...
func getPemMaterialFromDir(dir string) ([][]byte, error) {
	log.Logger.Debugf("Reading directory %s", dir)
	...
	for _, f := range files {
		...
		log.Logger.Debugf("Inspecting file %s", fullName)

		item, err := readPemFile(fullName)
		if err != nil {
			log.Logger.Warnf("Failed readgin file %s: %s", fullName, err)
			continue
		}
...
func getMspConfig(dir string, ID string, sigid *msp.SigningIdentityInfo) (*msp.MSPConfig, error) {
	...
	if os.IsNotExist(err) {
		log.Logger.Debugf("Intermediate certs folder not found at [%s]. Skipping. [%s]", intermediatecertsDir, err)
	} else if err != nil {
	...
	if os.IsNotExist(err) {
		log.Logger.Debugf("TLS CA certs folder not found at [%s]. Skipping and ignoring TLS intermediate CA folder. [%s]", tlsintermediatecertsDir, err)
	} else if err != nil {
		...
		if os.IsNotExist(err) {
			log.Logger.Debugf("TLS intermediate certs folder not found at [%s]. Skipping. [%s]", tlsintermediatecertsDir, err)
		} else if err != nil {
			return nil, fmt.Errorf("Failed loading TLS intermediate ca certs at [%s]: [%s]", tlsintermediatecertsDir, err)
		}
	} else {
		log.Logger.Debugf("TLS CA certs folder at [%s] is empty. Skipping.", tlsintermediatecertsDir)
	}

	crls, err := getPemMaterialFromDir(crlsDir)
	if os.IsNotExist(err) {
		log.Logger.Debugf("crls folder not found at [%s]. Skipping. [%s]", crlsDir, err)
	} else if err != nil {
	...
	// otherwise skip it
	var ouis []*msp.BlockchainOUIdentifier
	...
	if err == nil {
		...
		if len(configuration.OrganizationalUnitIdentifiers) > 0 {
			for _, ouID := range configuration.OrganizationalUnitIdentifiers {
				...
				oui := &msp.BlockchainOUIdentifier{
					Certificate:                  raw,
		...
		log.Logger.Debugf("MSP configuration file not found at [%s]: [%s]", configFile, err)
	}

	// Set BlockchainCryptoConfig
	cryptoConfig := &msp.BlockchainCryptoConfig{
		SignatureHashFamily:            bccsp.SHA2,
		IdentityIdentifierHashFunction: bccsp.SHA256,
	}

	// Compose BlockchainMSPConfig
	fmspconf := &msp.BlockchainMSPConfig{
		Admins:                        admincert,
		RootCerts:                     cacerts,
		IntermediateCerts:             intermediatecerts,
	...
	fmpsjs, _ := fmspconf.Marshal()

	mspconf := &msp.MSPConfig{Config: fmpsjs, Type: int32(BLOCKCHAIN)}
