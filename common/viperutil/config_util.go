...
func getKeysRecursively(base string, getKey viperGetter, nodeKeys map[string]interface{}, oType reflect.Type) map[string]interface{} {
	...
	for key := range nodeKeys {
		...
		if m, ok := val.(map[interface{}]interface{}); ok {
			log.Logger.Debugf("Found map[interface{}]interface{} value for %s", fqKey)
			...
			log.Logger.Debugf("Found map[string]interface{} value for %s", fqKey)
			result[key] = getKeysRecursively(fqKey+".", getKey, m, subTypes[key])
		} else if m, ok := unmarshalJSON(val); ok {
			log.Logger.Debugf("Found real value for %s setting to map[string]string %v", fqKey, m)
			result[key] = m
		} else {
			if val == nil {
			...
			log.Logger.Debugf("Found real value for %s setting to %T %v", fqKey, val, val)
			result[key] = val
...
func unmarshalJSON(val interface{}) (map[string]string, bool) {
	...
	s, ok := val.(string)
	if !ok {
		log.Logger.Debugf("Unmarshal JSON: value is not a string: %v", val)
		return nil, false
	}
	err := jsoniter.Unmarshal([]byte(s), &mp)
	if err != nil {
		log.Logger.Debugf("Unmarshal JSON: value cannot be unmarshalled: %s", err)
		return nil, false
	}
	return mp, true
}
...
func EnhancedExactUnmarshalFromGlobal(output interface{}) error {
	oType := reflect.TypeOf(output)
	if oType.Kind() != reflect.Ptr {
		return errors.Errorf("supplied output argument must be a pointer to a struct but is not pointer")
	}
	...
	getterWithClass := func(key string) interface{} { return viper.Get(key) } // hide receiver
	leafKeys := getKeysRecursively("", getterWithClass, baseKeys, eType)
