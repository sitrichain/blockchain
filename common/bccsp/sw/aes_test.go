...
// TestAESRelatedUtilFunctions tests various functions commonly used in blockchain wrt AES
...
func TestAESCBCPKCS7EncryptorDecrypt(t *testing.T) {
	...
	_, err = encryptor.Encrypt(k, msg, &MockEncrypterOpts{})
	assert.Error(t, err)
	...
	_, err = decryptor.Decrypt(k, ct, &MockEncrypterOpts{})
	assert.Error(t, err)
