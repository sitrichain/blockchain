...
func (ks *fileBasedKeyStore) storePrivateKey(alias string, privateKey interface{}) error {
	...
	if err != nil {
		log.Logger.Errorf("Failed converting private key to PEM [%s]: [%s]", alias, err)
		return err
	}

	err = ioutil.WriteFile(ks.getPathForAlias(alias, "sk"), rawKey, 0700)
	if err != nil {
		log.Logger.Errorf("Failed storing private key [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) storePublicKey(alias string, publicKey interface{}) error {
	...
	if err != nil {
		log.Logger.Errorf("Failed converting public key to PEM [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) storeKey(alias string, key []byte) error {
	...
	if err != nil {
		log.Logger.Errorf("Failed converting key to PEM [%s]: [%s]", alias, err)
		return err
	}

	err = ioutil.WriteFile(ks.getPathForAlias(alias, "key"), pem, 0700)
	if err != nil {
		log.Logger.Errorf("Failed storing key [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) loadPrivateKey(alias string) (interface{}, error) {
	...
	log.Logger.Debugf("Loading private key [%s] at [%s]...", alias, path)

	raw, err := ioutil.ReadFile(path)
	if err != nil {
		log.Logger.Errorf("Failed loading private key [%s]: [%s].", alias, err.Error())
	...
	if err != nil {
		log.Logger.Errorf("Failed parsing private key [%s]: [%s].", alias, err.Error())
...
func (ks *fileBasedKeyStore) loadPublicKey(alias string) (interface{}, error) {
	...
	log.Logger.Debugf("Loading public key [%s] at [%s]...", alias, path)

	raw, err := ioutil.ReadFile(path)
	if err != nil {
		log.Logger.Errorf("Failed loading public key [%s]: [%s].", alias, err.Error())
...
func (ks *fileBasedKeyStore) loadKey(alias string) ([]byte, error) {
	...
	log.Logger.Debugf("Loading key [%s] at [%s]...", alias, path)

	pem, err := ioutil.ReadFile(path)
	if err != nil {
		log.Logger.Errorf("Failed loading key [%s]: [%s].", alias, err.Error())
	...
	if err != nil {
		log.Logger.Errorf("Failed parsing key [%s]: [%s]", alias, err)
...
func (ks *fileBasedKeyStore) createKeyStoreIfNotExists() error {
	...
	if missing {
		log.Logger.Debugf("KeyStore path [%s] missing [%t]: [%s]", ksPath, missing, utils.ErrToString(err))

		err := ks.createKeyStore()
		if err != nil {
			log.Logger.Errorf("Failed creating KeyStore At [%s]: [%s]", ksPath, err.Error())
			return nil
		}
	}
...
func (ks *fileBasedKeyStore) createKeyStore() error {
	...
	ksPath := ks.path
	log.Logger.Debugf("Creating KeyStore at [%s]...", ksPath)

	os.MkdirAll(ksPath, 0755)

	log.Logger.Debugf("KeyStore created at [%s].", ksPath)
	return nil
}
...
func (ks *fileBasedKeyStore) openKeyStore() error {
	...
	log.Logger.Debugf("KeyStore opened at [%s]...done", ks.path)
