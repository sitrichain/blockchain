...
var (
	...
	curveHalfOrders = map[elliptic.Curve]*big.Int{
		...
		gm.P256Sm2():    new(big.Int).Rsh(gm.P256Sm2().Params().N, 1),
	}
)
...
func signECDSA(k *ecdsa.PrivateKey, digest []byte, _ bccsp.SignerOpts) (signature []byte, err error) {
	if k.Curve == gm.P256Sm2() {
		_, r, s, err := gm.Sm2Sign([]byte(""), digest, k)

		if err != nil {
			return nil, err
		}

		//logger.Debugf("\n sm2 sign digest: %s,r:%s,s:%s\n", hex.EncodeToString(digest), hex.EncodeToString(r.Bytes()), hex.EncodeToString(s.Bytes()))
...
func verifyECDSA(k *ecdsa.PublicKey, signature, digest []byte, _ bccsp.SignerOpts) (valid bool, err error) {
	r, s, err := UnmarshalECDSASignature(signature)
	if err != nil {
		return false, fmt.Errorf("Failed unmashalling signature [%s]", err)
	}
	//logger.Debugf("\n verifyECDSA sign digest: %s,r:%s,s:%s\n", hex.EncodeToString(digest), hex.EncodeToString(r.Bytes()), hex.EncodeToString(s.Bytes()))

	if k.Curve == gm.P256Sm2() {
		return gm.Sm2Verify([]byte(""), digest, k, signature), nil
	}
