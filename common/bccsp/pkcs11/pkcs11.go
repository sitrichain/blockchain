...
func loadLib(lib, pin, label string) (*pkcs11.Ctx, uint, *pkcs11.SessionHandle, error) {
	var slot uint = 0
	log.Logger.Debugf("Loading pkcs11 library [%s]\n", lib)
	if lib == "" {
	...
	for _, s := range slots {
		...
		log.Logger.Debugf("Looking for %s, found label %s\n", label, info.Label)
		if label == info.Label {
			found = true
			slot = s
			break
	...
	for i := 0; i < 10; i++ {
		...
		if err != nil {
			log.Logger.Warnf("OpenSession failed, retrying [%s]\n", err)
		} else {
			break
		}
	}
	if err != nil {
		log.Logger.Fatalf("OpenSession [%s]\n", err)
	}
	log.Logger.Debugf("Created new pkcs11 session %+v on slot %d\n", session, slot)
...
func (csp *impl) getSession() (session pkcs11.SessionHandle) {
	...
	case session = <-csp.sessions:
		log.Logger.Debugf("Reusing existing pkcs11 session %+v on slot %d\n", session, csp.slot)
...
func (csp *impl) getECKey(ski []byte) (pubKey *ecdsa.PublicKey, isPriv bool, err error) {
	...
	if err != nil {
		isPriv = false
		log.Logger.Debugf("Private key not found [%s] for SKI [%s], looking for Public key", err, hex.EncodeToString(ski))
	}
...
func (csp *impl) generateECKey(curve asn1.ObjectIdentifier, ephemeral bool) (ski []byte, pubKey *ecdsa.PublicKey, err error) {
	...
	ski = util.WriteAndSumSha256(ecpt)
	...
	log.Logger.Infof("Generated new P11 key, SKI %x\n", ski)
...
func (csp *impl) verifyP11ECDSA(ski []byte, msg []byte, R, S *big.Int, byteSize int) (valid bool, err error) {
	...
	log.Logger.Debugf("Verify ECDSA\n")
...
func (csp *impl) importECKey(curve asn1.ObjectIdentifier, privKey, ecPt []byte, ephemeral bool, keyType bool) (ski []byte, err error) {
	...
	if keyType == publicKeyFlag {
		log.Logger.Debug("Importing Public EC Key")
		publabel := fmt.Sprintf("BCPUB%s", id.Text(16))
		...
		log.Logger.Debugf("Importing Private EC Key [%d]\n%s\n", len(privKey)*8, hex.Dump(privKey))
		prvlabel := fmt.Sprintf("BCPRV%s", id.Text(16))
		keyTemplate = []*pkcs11.Attribute{
			pkcs11.NewAttribute(pkcs11.CKA_KEY_TYPE, pkcs11.CKK_EC),
			pkcs11.NewAttribute(pkcs11.CKA_CLASS, pkcs11.CKO_PRIVATE_KEY),
...
func (csp *impl) getSecretValue(ski []byte) []byte {
	...
	if err != nil {
		log.Logger.Warnf("P11: get(attrlist) [%s]\n", err)
	}
	...
	log.Logger.Warnf("No Key Value found!", err)
	return nil
}
