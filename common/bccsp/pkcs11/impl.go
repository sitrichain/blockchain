...
func (csp *impl) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	...
	case *bccsp.ECDSAPKIXPublicKeyImportOpts:
		...
		if csp.noPrivImport {
			...
			if !csp.softVerify {
				log.Logger.Debugf("opencryptoki workaround warning: Importing public EC Key does not store out to pkcs11 store,\n" +
	...
	case *bccsp.X509PublicKeyImportOpts:
		...
		if !ok {
			return nil, errors.New("[X509PublicKeyImportOpts] Invalid raw material. Expected *gm.Certificate.")
		}
...
func (csp *impl) Encrypt(k bccsp.Key, plaintext []byte, opts bccsp.EncrypterOpts) (ciphertext []byte, err error) {
	// TODO: Add PKCS11 support for encryption, when blockchain starts requiring it
