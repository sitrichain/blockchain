...
func PrivateKeyToDER(privateKey *ecdsa.PrivateKey) ([]byte, error) {
	...
	return gm.MarshalECPrivateKey(privateKey)
}
...
func PrivateKeyToPEM(privateKey interface{}, pwd []byte) ([]byte, error) {
	...
	case *ecdsa.PrivateKey:
		...
		// based on https://golang.org/src/crypto/gm/sec1.go
...
func PrivateKeyToEncryptedPEM(privateKey interface{}, pwd []byte) ([]byte, error) {
	...
	case *ecdsa.PrivateKey:
		...
		raw, err := gm.MarshalECPrivateKey(k)

		if err != nil {
			return nil, err
		}

		block, err := gm.EncryptPEMBlock(
			rand.Reader,
			"PRIVATE KEY",
			raw,
			pwd,
...
func DERToPrivateKey(der []byte) (key interface{}, err error) {
	...
	if key, err = gm.ParsePKCS1PrivateKey(der); err == nil {
		return key, nil
	}

	if key, err = gm.ParsePKCS8PrivateKey(der); err == nil {
		switch key.(type) {
	...
	if key, err = gm.ParseECPrivateKey(der); err == nil {
		return
	}
...
func PEMtoPrivateKey(raw []byte, pwd []byte) (interface{}, error) {
	...
	if gm.IsEncryptedPEMBlock(block) {
		if len(pwd) == 0 {
			return nil, errors.New("Encrypted Key. Need a password")
		}

		decrypted, err := gm.DecryptPEMBlock(block, pwd)
		if err != nil {
...
func PublicKeyToDER(publicKey interface{}) ([]byte, error) {
	...
	case *ecdsa.PublicKey:
		...
		PubASN1, err := gm.MarshalPKIXPublicKey(k)
		if err != nil {
			return nil, err
		}
...
func PublicKeyToEncryptedPEM(publicKey interface{}, pwd []byte) ([]byte, error) {
	...
	case *ecdsa.PublicKey:
		...
		raw, err := gm.MarshalPKIXPublicKey(k)
		if err != nil {
			return nil, err
		}
...
func DERToPublicKey(raw []byte) (pub interface{}, err error) {
	...
	key, err := gm.ParsePKIXPublicKey(raw)
